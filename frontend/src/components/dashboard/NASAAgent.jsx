...
import { Send, Sparkles, Rocket, Satellite, Image as ImageIcon, Loader2, Map as MapIcon, Layers, Square, Pentagon, Globe, Mountain, Navigation } from 'lucide-react';
...
export default function NASAAgent() {
  ...
  const [currentTypingMessageIndex, setCurrentTypingMessageIndex] = useState(null);
  const [mapStyle, setMapStyle] = useState('dark'); // 'dark', 'light', 'streets', 'satellite', 'terrain'
  // Map selection state
      ...
      if (data && data.data) {
        ...
        if (showMessage) {
          const layerDescriptions = {
            'heatmap': '**Urban Heat Island Analysis**\n\n📡 **Data Source:** NASA MODIS Land Surface Temperature\n\n🔥 Red areas indicate higher surface temperatures. This visualization helps identify heat stress zones and prioritize cooling interventions like green spaces and reflective surfaces.',
            'scatterplot': '**Air Quality Monitoring**\n\n📡 **Data Sources:** NASA AIRS & OMI Sensors\n\n💨 Air quality monitoring stations with AQI levels. Larger circles indicate worse air quality. Use this data to identify pollution hotspots and plan mitigation strategies.',
            'line': '**Urban Transportation Network**\n\n📡 **Data Sources:** OpenStreetMap & NASA VIIRS\n\n🚗 Transportation corridors and connectivity analysis. Colors show traffic intensity patterns. Essential for infrastructure planning and congestion management.',
            '3d-scatterplot': '**3D Urban Density Map**\n\n📡 **Data Sources:** Landsat + OpenStreetMap\n\n🏙️ Building heights and urban density visualization. Rotate and pitch the view to see 3D elevation. Useful for skyline analysis and density planning.'
          };
          
          const visualizationMessage = {
            role: 'assistant',
            content: layerDescriptions[layerType] || `Displaying ${layerType} visualization`,
            timestamp: new Date().toISOString(),
            isMarkdown: true,
            api_sources: [
              { name: 'NASA EOS', description: 'Earth Observing System' },
              { name: layerType === 'heatmap' ? 'MODIS' : layerType === 'scatterplot' ? 'AIRS/OMI' : layerType === 'line' ? 'VIIRS' : 'Landsat', description: 'Satellite sensor data' }
            ]
          };
          
          setMessages(prev => {
            const newMessages = [...prev, visualizationMessage];
            // Mark this message for typing animation
            setCurrentTypingMessageIndex(newMessages.length - 1);
            return newMessages;
          });
          
          // After typing completes (estimate based on content length)
          setTimeout(() => {
            setCurrentTypingMessageIndex(null);
          }, visualizationMessage.content.length * 30 + 1000);
        }
    ...
    try {
      const response = await fetch(`${NASA_AGENT_API}/chat`, {
        ...
        body: JSON.stringify({
          ...
          boundary: selectedArea // Include selected area if available
        })
      });
      ...
      // Disable typing after animation completes
      setTimeout(() => {
        setCurrentTypingMessageIndex(null);
      }, data.content.length * 30 + 1000);
  ...
  const renderToolOutput = (toolName, output) => {
    try {
      if (toolName === 'get_city_satellite_imagery' || toolName === 'get_mars_rover_photos') {
        return renderImageFromData(output);
      }
      
      if (toolName === 'get_natural_disasters' && output.events) {
        return (
          <div className="mt-2 space-y-1">
            {output.events.map((event, idx) => (
              <div key={idx} className="text-xs bg-red-900/30 border border-red-500/30 rounded p-2">
                <p className="font-semibold text-red-200">{event.title}</p>
                <p className="text-red-300">{event.category} - Magnitude: {event.magnitude}</p>
              </div>
            ))}
          </div>
        );
      ...
      if (toolName === 'search_asteroids' && output.asteroids) {
        return (
          <div className="mt-2 space-y-1">
            {output.asteroids.slice(0, 3).map((asteroid, idx) => (
              <div key={idx} className="text-xs bg-orange-900/30 border border-orange-500/30 rounded p-2">
                <p className="font-semibold text-orange-200">{asteroid.name}</p>
                <p className="text-orange-300">
                  Distance: {asteroid.distance} km | 
                  Hazardous: {asteroid.is_potentially_hazardous ? '⚠️ Yes' : '✅ No'}
                </p>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-slate-950 via-blue-950/30 to-slate-900 flex relative overflow-hidden">
      ...
      <div className="flex-1 relative z-10">
        {showMap && (
          <Card className="h-full bg-slate-900/50 backdrop-blur-sm border-blue-500/30 m-2 flex flex-col overflow-hidden shadow-2xl shadow-blue-500/20">
            <div className="flex-shrink-0">
              {/* Map controls with scanning effect */}
              <div className="relative overflow-hidden">
                ...
                <div className="p-3 bg-gradient-to-r from-slate-900/90 via-blue-900/50 to-slate-900/90 border-b border-blue-500/40 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <MapIcon className="w-4 h-4 text-blue-400" />
                    <span className="text-blue-200 text-xs font-bold">NASA EOS</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button
                        ...
                        disabled={loading}
                        className={`text-xs ${selectedLayerType === 'scatterplot' ? 'bg-gradient-to-r from-orange-600 to-yellow-600 shadow-lg shadow-orange-500/50 scale-105' : 'bg-orange-900/50 hover:scale-105'} hover:bg-orange-700 border border-orange-500/30 text-orange-200 transition-all duration-300 transform font-semibold`}
                      >
                        💨 Air
                      </Button>
                      <Button
                    ...
                    {/* Map Style Selector */}
                    <div className="h-6 w-px bg-blue-500/30 mx-1"></div>
                    <div className="flex gap-1">
                      <Button
                        onClick={() => setMapStyle('dark')}
                        className={`text-xs ${mapStyle === 'dark' ? 'bg-slate-700 scale-105' : 'bg-slate-900/50'} hover:bg-slate-700 border border-slate-500/30 text-slate-200 transition-all`}
                        title="Dark Mode"
                      >
                        🌙
                      </Button>
                      <Button
                        ...
                        className={`text-xs ${mapStyle === 'streets' ? 'bg-blue-600 scale-105' : 'bg-blue-900/50'} hover:bg-blue-700 border border-blue-500/30 text-blue-200 transition-all`}
                        title="Streets"
                      >
                      ...
                      <Button
                        ...
                        className={`text-xs ${mapStyle === 'satellite' ? 'bg-green-600 scale-105' : 'bg-green-900/50'} hover:bg-green-700 border border-green-500/30 text-green-200 transition-all`}
                        title="Satellite"
                      >
                      ...
                      <Button
                        ...
                        className={`text-xs ${mapStyle === 'terrain' ? 'bg-amber-600 scale-105' : 'bg-amber-900/50'} hover:bg-amber-700 border border-amber-500/30 text-amber-200 transition-all`}
                        title="Terrain 3D"
                      >
              ...
              <div className="flex-1 relative h-[calc(100vh-120px)]">
                {visualizationData ? (
                  <MapVisualization
                    ...
                    locationLabels={mapLabels}
                    mapStyle={mapStyle}
                  />
                ) : (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/50">
                    <Layers className="w-16 h-16 text-blue-400/50 mb-4" />
                    <p className="text-blue-300 text-center px-8">
                      Click a layer button above to visualize NASA satellite data
                    </p>
                  </div>
                )}
                
                {/* Map Selection Tools - Temporarily disabled (nebula.gl incompatible with deck.gl 9.1.14) */}
                {/* TODO: Re-implement with mapbox-gl-draw or deck.gl 8.x */}
                {false && (
                <div className="absolute bottom-4 left-4 z-20 flex gap-2">
                  <Button
                    onClick={() => setSelectionMode(selectionMode === 'rectangle' ? null : 'rectangle')}
                    className={`text-xs ${selectionMode === 'rectangle' ? 'bg-green-600 scale-105' : 'bg-slate-800/80'} hover:bg-green-700 border border-green-500/30 text-green-200 transition-all transform`}
                    title="Draw rectangle to select area"
                  >
                    <Square className="w-4 h-4" />
                  </Button>
                  <Button
                    onClick={() => setSelectionMode(selectionMode === 'polygon' ? null : 'polygon')}
                    className={`text-xs ${selectionMode === 'polygon' ? 'bg-green-600 scale-105' : 'bg-slate-800/80'} hover:bg-green-700 border border-green-500/30 text-green-200 transition-all transform`}
                    title="Draw polygon to select area"
                  >
                  ...
                  {selectedArea && (
                    <>
                      <Button
                        onClick={() => {
                          const coords = selectedArea.geometry.coordinates[0];
                          setInput(`Analyze this selected area with ${coords.length} boundary points`);
                        }}
                        className="text-xs bg-blue-600 hover:bg-blue-700 border border-blue-500/30 text-blue-200"
                      >
                      ...
                      <Button
                        ...
                        className="text-xs bg-red-600 hover:bg-red-700 border border-red-500/30 text-red-200"
                      >
                        Clear Selection
                      </Button>
                    </>
            ...
            <div key={index} className="flex flex-col animate-slide-in">
              ...
              >
                <div className="p-3">
                  ...
                  {/* API Sources badges */}
                  {message.api_sources && message.api_sources.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-2">
                      {message.api_sources.map((api, idx) => (
                        <span 
                          key={idx}
                          className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-500/20 text-blue-300 border border-blue-400/30"
                  ...
                  {message.isMarkdown ? (
                    <MarkdownMessage 
                      ...
                      isTyping={currentTypingMessageIndex === index}
                      speed={30}
                    />
                  ) : (
              ...
              <div className="grid grid-cols-2 gap-1">
                <Button
                  onClick={() => {
                    ...
                    handleVisualizationRequest('heatmap', true);
                  }}
                ...
                <Button
                  onClick={() => {
                    ...
                    handleVisualizationRequest('scatterplot', true);
                  }}
                ...
                <Button
                  onClick={() => {
                    ...
                    handleVisualizationRequest('line', true);
                  }}
                ...
                <Button
                  onClick={() => {
                    setInput("Show 3D urban density and building heights");
                    handleVisualizationRequest('3d-scatterplot', true);
                  }}
                  ...
                  disabled={loading || isTyping}
                >
                  <Layers className="w-3 h-3 mr-1 animate-pulse" />
                  3D Buildings
                </Button>
              </div>
            </div>
        ...
        .stars-3d, .stars-3d-2, .stars-3d-3 {
          ...
          box-shadow: 
            ${Array.from({length: 100}, () => 
              `${Math.random() * 2000}px ${Math.random() * 1000}px ${Math.random() * 2}px ${Math.random()}px rgba(255,255,255,${Math.random()})`
            ).join(',')};
        }
        
        .stars-3d {
          animation: stars-3d-move 120s linear infinite;
        }
        
        .stars-3d-2 {
          animation: stars-3d-move 180s linear infinite;
          animation-delay: -60s;
        }
        
        .stars-3d-3 {
          animation: stars-3d-move 240s linear infinite;
          animation-delay: -120s;
        }
        
        @keyframes stars-3d-move {
          from { transform: translateZ(0) translateY(0); }
          to { transform: translateZ(1000px) translateY(1000px); }
        }
